...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QPushButton,
                             ...
                             QFrame, QApplication, QToolTip, QProgressBar,
                             QRadioButton)
from PyQt6.QtCore import Qt, pyqtSignal, QDateTime, QDate, QTime, QTimer, QThread
from PyQt6.QtGui import QPixmap, QImage, QColor, QPainter, QPen

from PyQt6.QtCharts import QChart, QChartView, QLineSeries, QValueAxis, QDateTimeAxis, QScatterSeries
...
class SyncThread(QThread):
    ...
    def __init__(self, wb_codes, manager, cancelled_ref):
        super().__init__()
        self.wb_codes = wb_codes
    ...
    def run(self):
        ...
        success = 0
        fail = 0
        for i, wb_code in enumerate(self.wb_codes):
            ...
            for url in url_configs:
                ...
                try:
                    ...
                    if img_data and len(img_data) > 1000:
                        self.manager.update_image(wb_code, img_data)
                        success += 1
                        downloaded = True
                        break
                except:
...
class CompetitorDialog(QDialog):
    ...
    def sync_images(self):
        """同步当前商品所有竞品的主图（竞品对话框按钮，去掉确认弹窗，直接下载）"""
        import urllib.request
        import os
        ...
        self._sync_cancelled_ref = [False]
        self._sync_thread = ImgThread(wb_codes, self.manager, self._sync_cancelled_ref)
    ...
    def _extract_wb_code(self, text: str) -> str:
        ...
        # 情况2：从链接中提取 /catalog/ 后面的数字
        # 匹配模式：/catalog/数字/ 或 /catalog/数字? 或 catalog/数字/
        patterns = [
